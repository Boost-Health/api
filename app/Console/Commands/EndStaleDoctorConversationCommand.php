...
class EndStaleDoctorConversationCommand extends Command
{
    ...
    public function handle(): void
    {
        ...
        foreach ($doctorsWithActiveConversations as $doctorWithActiveConversation) {
            $this->info('stale:conversation:processing', ['doctor_id' => $doctorWithActiveConversation->id]);
    ...
    private function check(User $doctor): void
    {
        ...
        $doctorParticipant = $doctor->activeConversation->getParticipants()->reject(fn ($participant) => $participant->user->is($doctor))->first();
        $userParticipant = $doctor->activeConversation->getParticipants()->filter(fn ($participant) => $participant->user->is($doctor))->first();
        ...
        if ($lastMessage?->created_at->diffInHours(now()) >= self::NUMBER_OF_HOURS_TO_CHECK_FOR_STALE) {
            ...
            $this->info('stale:conversation:ended', ['conversation_id' => $conversationId]);
        }
    }
}
